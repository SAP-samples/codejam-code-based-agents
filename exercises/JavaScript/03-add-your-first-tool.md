...
> - The constructor accepts a model name (`'sap-rpt-1-small'` or `'sap-rpt-1-large'`) and defaults to `'sap-rpt-1-small'`.
...
The `@sap-ai-sdk/rpt` client resolves the RPT-1 deployment by **model name** — no deployment URL is needed. The `RPT1Client` constructor accepts `'sap-rpt-1-small'` (default) or `'sap-rpt-1-large'`.

👉 Make sure your `.env` file contains valid SAP AI Core credentials (`AICORE_SERVICE_KEY`) and your `RESOURCE_GROUP` is set to `ai-agents-codejam`.
...
### Step 3: Update buildGraph() and Pass the Payload

Now that `payload` is part of `AgentState`, you also need to declare it as a channel in the `StateGraph` so LangGraph knows to track it.

👉 Update the `buildGraph()` function in `basicAgent.ts` to include the `payload` channel:
...
👉 Update `main()` in `basicAgent.ts` to pass the payload as part of the initial state:
