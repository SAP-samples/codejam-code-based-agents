...
> 💡 **Key insight:** Joule doesn't need to know that your agent uses CrewAI, Python, or any specific framework. It only speaks A2A — the same protocol you set up in Exercise 08. The capability YAML is just the bridge between Joule's chat UI and your A2A endpoint.
...
> ⚠️ **Replace `<your-app-route>`** with the actual route of your CF app from Exercise 08. You can find it by running `cf app investigator-crew-a2a` or by checking the URL you noted at the end of Exercise 08.
...
> 💡 **This maps directly to what you built in Exercise 08.** In `server.py`, your `InvestigatorExecutor` emits a `TaskArtifactUpdateEvent` with `artifactId="investigation_result"` and a `TextPart`. That's exactly what Joule reads here through `artifacts[0].parts[0].text`.
